...
static ListNode append(ListNode a, ListNode b){
...
ListNode newNode(char *v){
    ...
    ListNode new = malloc(sizeof(Node_url));
    new->next = NULL;
    new->url = v;
    return new;
...
{
    if (t == NULL){
        BSTLink new = newBSTNode(v);
        new->urlList = append(new->urlList, newNode(url));
        return new;
    }
    else if (equ(v, t->value) < 0){
        ...
        t->urlList = append(t->urlList, newNode(url));
    }
