...
/*
static int cmp(Tree t, Key a, Key b);
static int eq(Tree t, Key a, Key b);
static int less(Tree t, Key a, Key b);
*/
...
static int cmp1(Link t, Key a, Key b){

    t->within->ncompares++;
    return (a-b);

}
static int eq(Link t, Key a, Key b){
    t->within->ncompares++;
    if(a == b){
        return 1;
    }else{
...
static int less(Link t, Key a, Key b){
...
Link insert(Link t, Item it)
{
	...
	int diff = cmp1(t ,key(it),key(t->value));
	if (diff == 0)
		t->value = it;
	else if (diff < 0)
...
static Link searchSplay(Link t, Key k, int *found)
{
	...
	else if (eq(t,key(t->value),k)) {
		*found = 1; // item found, store true  
		res =  t;  
	}
	else if (less(t,k,key(t->value))) {
		if (t->left == NULL){
			*found = 0;// item not found
			//res = rotateRight(t); 
			res = t;
		}
		else if (eq(t,key(t->left->value),k)) {
			*found = 1;
			res = rotateR(t);
		}
		else {
			if (less(t,k,key(t->left->value))) {
				// left-left
		...
		else if (eq(t,key(t->right->value),k)) {
			*found = 1;
			res = rotateL(t);
		}
		else{
			if (less(t,key(t->right->value),k)) {
				/* right-right */
