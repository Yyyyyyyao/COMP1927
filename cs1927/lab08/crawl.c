...
int main(int argc, char **argv)
{
	...
	while(!emptyStack(ToDolist) && nVertices(g) < maxURLs){
		char *x = popFrom(ToDolist);

		strcpy(baseC, baseURL);

		if(strstr(x, "/A/index") != NULL){
			strcat(baseC, "/A");
		}
		if(strstr(x, "/B/index") != NULL){
			strcat(baseC, "/B");
		}

		if (!(handle = url_fopen(x, "r"))) {
		...
		while(!url_feof(handle)) {
			...
			while ((cur = nextURL(cur)) != NULL) {
				...
				if(nVertices(g) < maxURLs && isConnected(g, x, full_link) != 1){
					addEdge(g, x, full_link);
				}
                if(!isElem(visited, full_link)){
                    insertInto(visited, full_link);
                    pushOnto(ToDolist, full_link);
                    printf("Found %s\n",full_link);
